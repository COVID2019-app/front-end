...
function Country(props) {
    ...
    const { getRegionSum, getCountryRegions, region, country, region_sum, isFetching } = props;
    ...
    useEffect(() => {
        ...
        getCountryRegions(country_name);
        getRegionSum(country_name)
    }, [getCountryRegions, getRegionSum, country_name])
    ...
    for (var j in region_cases) {
        if (new Date(region_cases[j].date) !== "Invalid Date") {
            region_cases[j].date = new Date(region_cases[j].date).toLocaleString("en-US", {timeZone: 'Asia/Brunei' })
        }
        if(data.deaths){
            if (new Date(region_deaths[j].date) !== "Invalid Date") {
                region_deaths[j].date = new Date(region_deaths[j].date).toLocaleString("en-US", { timeZone: 'Asia/Brunei' })
            }
        }
    }
    ...
    return (
        <div style={{ textAlign: 'center' }}>
            ...
            <br />
            <CommonChart region_sum={region_sum} region_data={region_cases} region_names={region_names} country_name={data.country} isFetching={isFetching} field="confirmed_cases" title="Cases" />
            <br />
            {
                data.deaths 
                ? 
                (
                    ...
                    <br />
                    <CommonChart region_sum={region_sum} region_data={region_deaths} region_names={region_names} country_name={data.country} isFetching={isFetching} field="deaths" title="Deaths" />
                    </React.Fragment>
                )
                :
                (<div></div>)
...
export default withRouter(connect(mapStateToProps, { getCountryRegions, getRegionSum })(Country));